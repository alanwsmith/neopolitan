...
pub fn wc(source: &str) -> IResult<&str, Section> {
...
mod test {
    ...
    #[test]
    fn basic_widget() {
        let lines = vec!["-> wc", ">> component-name", "", "some data"].join("\n");
        let source = lines.as_str();
        let expected = Wrapper::Page {
            children: Some(vec![Section::WebComponentSection {
                attributes: Some(vec![SectionAttribute::Attribute {
                    key: Some("component-name".to_string()),
                    value: None,
                }]),
                children: Some(Block::RawContent {
                    text: Some("some data".to_string()),
                }),
            }]),
        };
