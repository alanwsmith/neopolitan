...
pub fn title_with_paragraphs() {
    // NOTE: The final strings are collapsed and white
    // space removed for testing to make things
    // easier to work with
    let mut u = Universe::new();
    u.env = Some(create_env("./src/tests/templates"));
    let lines = ["-> title", "", "Pick The Rose", "", "Add salt"];
    let expected = Some(vec![
        r#"<h1 class="title">Pick The Rose</h1>"#.to_string(),
        r#"<p>Add salt</p>"#.to_string(),
    ]);
