...
with open(output_path, "w") as _out:
    ...
    lines.append("")
    sql = 'SELECT tag FROM sections WHERE type=? AND rust_template IS NOT NULL ORDER BY tag'
    ...
    lines.append("")
    sql = 'SELECT tag FROM sections WHERE type=? AND rust_template IS NULL ORDER BY tag'
    for row in cur.execute(sql, ("content",)):
        lines.append(f"- <<{row[0]}|link|/sections/{row[0]}.html>>")
        lines.append("")
    _out.write("\n".join(lines))
