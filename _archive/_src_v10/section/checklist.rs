use crate::block::checklist_item::*;
...
pub fn checklist(source: &str) -> IResult<&str, Section> {
    ...
    let (source, b) = many_till(checklist_item, eof)(source.trim()).unwrap();
...
mod test {
    ...
    #[test]
    fn checklist_implementation() {
        let lines = vec!["-> checklist", "", "[] item alfa", "", "[] item bravo"].join("\n");
        let source = lines.as_str();
        let expected = Wrapper::Page {
            children: Some(vec![Section::ChecklistSection {
                attributes: None,
                children: Some(vec![
                    Block::ChecklistItem {
                        status: None,
